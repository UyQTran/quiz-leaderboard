...
i fredagsquizene hennes, kan de få minuspoeng. Slik at den totale poengsummen hver ansatt har kan gå 
både opp og ned. Hun trenger derfor et rangeringssystem slik at hun kan holde oversikt over hvem som 
...
EventStream-tab. Det er nemlig her du kan se sanntidsdataene som blir sendt fra backend. Holde denne tabben oppe på
nettleseren din siden vi kommer til å bruke den mye fremover.

På grunn av HTTP begrensninger, er det lurt å utnytte samme SSE strøm til forskjellig type data. Det er derfor det er 
en mulighet for å sette Type som metadata i strømmen slik at frontend kun trenger å lytte til én strøm og reagere 
forskjellig utifra hva slags type data den får inn.

### Singals
Nå som vi kan få strømmet sanntidsdata mellom backend og frontend, kan vi videre strømme dataene videre til
...
Nå har vi definert en Angular-service som henter data fra backend. Disse kan nå hentes av Angular-komponenter. 
I `leaderboard.ts` kan vi injisere `DataFetcher`-servicen vår ved å legge til følgende i klassen
``` typescript
...
til datamaskinen med x604,800 for å øke effektiviteten og dermed tjene mer penger. Dette betyr at hver uke for en StepWeb
ansatt er like mye som ett sekund for oss utenfor datamaskinen. Med andre ord så skjer fredagsquiz nå hvert sekund. Vi må
nå skrive litt mer logikk som håndterer hyppige oppdateringer av data i Rika Rangering.
...
eksisterende `Ranking`-elementet. Et lite hint her er at du må sende inn noe annet enn en tom liste som initiell verdi til 
`reduce` funksjonen
